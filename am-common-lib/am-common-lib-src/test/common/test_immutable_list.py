...
def test_builtin_list_has_no_new_members() -> None:
    expected_members = {
        "__add__",
        "__class__",
        "__class_getitem__",
    ...
    assert_that(set(dir(list))).is_equal_to(expected_members)


def test_non_overriden_attributes() -> None:
    not_overridden = get_not_overridden_attributes(ImmutableList, list)
    ...
    expected_safe_attributes = {
        ...
        "__gt__",
        "__ge__",
        # Concatenation and multiplication (return new lists)
        "__add__",
        "__mul__",
        "__rmul__",
        # Copy operation
